...
const navItems = [
  { href: "/dashboard/nino/inicio", label: "Inicio", emoji: "🏠" },
  { href: "/dashboard/nino/cursos", label: "Mis Cursos", emoji: "📚" },
  { href: "/dashboard/nino/mentores", label: "Mis Mentores", emoji: "🤖" },
  { href: "/dashboard/nino/progreso", label: "Mi Progreso", emoji: "🏆" },
];
...
export default function NinoSidebar({ childName, childEmoji }: Props) {
  const pathname = usePathname();
  const [mobileOpen, setMobileOpen] = useState(false);

  const content = (
    <nav className="h-full flex flex-col">
      {/* Logo + avatar del niño */}
      <div className="px-6 py-6 border-b border-white/20">
        ...
        <div className="flex items-center gap-3 bg-white/10 rounded-2xl px-3 py-2">
          <span className="text-3xl">{childEmoji}</span>
          <div>
            <p className="font-body font-semibold text-white text-sm">{childName}</p>
            <p className="font-body text-xs text-white/60">Modo aprendizaje 🎓</p>
          </div>
        </div>
      </div>
        ...
        {navItems.map(({ href, label, emoji }) => {
          ...
          return (
            <Link
              ...
              className={cn(
                "flex items-center gap-3 px-4 py-3.5 rounded-2xl font-body font-semibold text-sm transition-all",
                isActive
